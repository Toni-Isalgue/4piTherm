...
//#include <windows.h>
int main(void)  {
double rr2,co,si,si2,cocont,sicont,co2,Tmin,Tmax,T2k,T2avk,T2avsk,T2gn,T2cil,sinaa,cosaa,coef,coefcil;
double T2gav,xx,yy,r2,T2g,T2,T2av,T2avs,acocont,asicont,bgr,Tair,RadS,angle,transmit,absorb,v;
...
//  air temperature, wind speed, radiation on horizontal plane (meteodat.dat), angle of sun elevation (º) over horizontal, transmitance
//       Tair, v, RadS, angle, transmit, absorb
//                              (assuming there could be some partial shadow by trees, for instance), and absortance of solar radiation by the subject/cylinder 
   ...
   printf("Meteo and properties: %f %f %f %f %f %f \n", Tair,v,RadS,angle,transmit,absorb);  //include air speed v
...
// results average Trad for sphere (point)  -> and expected Tglobe from meteo
...
fprintf(PunteroDatos, " \n");
fprintf(PunteroDatos, "Meteo and other data: Tair, vair, RadS, angle, transmit, absorb %f %f %f %f %f %f \n",Tair,v,RadS,angle, transmit, absorb);
fprintf(PunteroDatos, " \n");
fprintf(PunteroDatos, "No direct Sun, from IR pictures Imagez and Imagen: \n");
fprintf(PunteroDatos, " \n");
fprintf(PunteroDatos, "Equivalent Radiant temperature for a sphere: %f (K)    %f (ºC) \n", T1gn, T1gn-273.15); //add Tglobe estimation
fprintf(PunteroDatos, " \n");
coef=4*0.98*0.0000000667*pow(((T1gn+Tair+273.15)/2),3); //0.98 appoach to 1 or/and 0.95
coef=(coef*(T1gn-273.15)+(5.7+3.8*v)*Tair)/(coef+5.7+3.8*v);
fprintf(PunteroDatos, "Approximate value for measured black globe temperature in K and in ºC: %f %f \n", 273.15+coef, coef);
fprintf(PunteroDatos, " \n");
fprintf(PunteroDatos, "Equivalent Radiant temperature for a cylinder 0.88m2 lateral, 0.06 m2 up: %f (K)   %f (ºC) \n", T1cil,T1cil-273.15);
fprintf(PunteroDatos, " \n");
coefcil=4*absorb*0.0000000667*pow(((Tair+273.15+T1cil)/2),3);
coefcil=(coefcil*(T1cil-273.15)+(5.7+3.8*v)*Tair)/(coefcil+5.7+3.8*v);
fprintf(PunteroDatos, "Approximate value for measured cylinder (absorb) temperature in K and in ºC: %f %f \n", 273.15+coefcil, coefcil);
fprintf(PunteroDatos, " \n");
fprintf(PunteroDatos, " \n");
...
fprintf(PunteroDatos, " \n");
fprintf(PunteroDatos, "Equivalent Radiant temperature for a sphere under Sunlight: %f (K)    %f (ºC) \n", T2gn, T2gn-273.15);  //add Tglobe estimation
fprintf(PunteroDatos, " \n");
coef=4*0.98*0.0000000667*pow(((T2gn+Tair+273.15)/2),3); //0.98 appoach to 1 or/and 0.95 //Attention surface to Sun and total surface
coef=(coef*(T2gn-273.15)+(5.7+3.8*v)*Tair)/(coef+5.7+3.8*v);
fprintf(PunteroDatos, "Approximate value for measured black globe temperature in K and in ºC: %f %f \n", coef+273.15, coef);
fprintf(PunteroDatos, " \n");
fprintf(PunteroDatos, "Equivalent Radiant temperature for a cylinder 0.88m2 lateral, 0.06 m2 up under Sunlight: %f (K)   %f (ºC) \n", T2cil,T2cil-273.15);
fprintf(PunteroDatos, " \n");
coefcil=4*absorb*0.0000000667*pow(((T2cil+Tair+273.15)/2),3); //0.98 appoach to 1 or/and 0.95
coefcil=(coefcil*(T2cil-273.15)+(5.7+3.8*v)*Tair)/(coefcil+5.7+3.8*v);
fprintf(PunteroDatos, "Approximate value for measured cylinder (absorb) temperature in K and in ºC: %f %f \n", 273.15+coefcil, coefcil);
fprintf(PunteroDatos, " \n");
fprintf(PunteroDatos, " \n");
